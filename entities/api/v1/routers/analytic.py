...
async def get_recommended_cars(
    ...
    query = text("""
        ...
        WHERE recommendation_status = 'RECOMMENDED'
          ...
          AND (array_length(:vehicle_condition, 1) = 0 OR ca.issue_description = ANY(:vehicle_condition))
          AND (array_length(:vehicle_types, 1) = 0 OR vehicle_type = ANY(:vehicle_types))
          ...
          AND (array_length(:engine_type, 1) = 0 OR engine = ANY(:engine_type))
          AND (array_length(:transmission, 1) = 0 OR transmission = ANY(:transmission))
          AND (array_length(:drive_train, 1) = 0 OR drive_train = ANY(:drive_train))
          AND (array_length(:cylinder, 1) = 0 OR engine_cylinder = ANY(:cylinder))
          AND (array_length(:auction_names, 1) = 0 OR auction_name = ANY(:auction_names))
          AND (array_length(:body_style, 1) = 0 OR body_style = ANY(:body_style))
        LIMIT 50;
    """)
